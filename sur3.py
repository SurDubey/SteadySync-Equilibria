...
class ExampleMoveItTrajectories(object):
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/kinova_arm/camera/color/image_raw", Image, self.image_callback)
    ...
    def save_image(self, location_id):
        """Save the latest image with a unique filename"""
        if self.latest_image is not None:
            filename = os.path.join(self.image_save_path, f"location_{location_id}.jpg")
            cv2.imwrite(filename, self.latest_image)
            rospy.loginfo(f"Saved image at {filename}")
        else:
    ...
    if success:
        ...
        for i, location in enumerate(locations):
            ...
            if success:
                ...
                rospy.sleep(2)  # Wait for the arm to stabilize

                # Capture and save the image
                example.save_image(i+1)

                rospy.sleep(3)  # Stay at the location for 3 more seconds
            else:
